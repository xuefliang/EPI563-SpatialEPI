...
```{r }
pacman::p_load(
  tidyverse, # for general data management
  ...
  raster
) # The outputs of these KDE functions will be raster. This package gives us tools for working with rasters
```
...
```{r eval = F}
death_kde_adapt <- bivariate.density(
  d_ppp,
  h0 = h_os_d,
  edge = 'diggle',
  adapt = TRUE,
...
birth_kde_adapt <- bivariate.density(
  b_ppp,
  h0 = h_os_b,
  edge = 'diggle',
  adapt = TRUE,
...
make_map <- function(x, bw) {
  ...
  tm_shape(x$rr) +
    ...
    tm_shape(x$plines) +
    tm_lines(col = 'level', legend.col.show = F) +
    tm_layout(main.title = mtitle, legend.format = list(digits = 1))
...
```{r}
在调用函数前，先看看 8000 对应的栅格数据长什么样：R# 查看数据摘要
summary(values(raster(imr8000$P)))

# 或者直接看最大最小值
range(values(raster(imr8000$P)), na.rm = TRUE)
如果 range 不包含 $0.025$ 或 $0.975$，报错就是必然的。
```
...
atl <- as(atl_sf, "Spatial") # convert to sp class (required for GWmodel)
```
...
tm_shape(atl_sf) +
  tm_polygons(
    fill = c('pctNOHS', 'pctPOV', 'pctMOVE', 'pctOWNER_OCC'),
    fill.scale = tm_scale_intervals(style = 'quantile'), # 适配 tmap v4 语法
    fill.free = TRUE # 允许每个子图有独立的颜色标尺
  ) +
  tm_facets(ncol = 2) # 排版成 2 列
```
...
```{r message = F, warning = F}
tm_shape(atl_sf) +
  tm_fill('ICE_INCOME_all', style = 'quantile', palette = 'div') +
  tm_borders()
```
...
bw.gwss.average(
  atl,
  vars = c('pctPOV', 'pctPOV', 'pctMOVE', 'pctOWNER_OCC', 'ICE_INCOME_all')
)
```
...
bw.gwss.average(
  atl,
  vars = c('pctPOV', 'pctNOHS', 'pctMOVE', 'pctOWNER_OCC', 'ICE_INCOME_all'),
  adaptive = T
)
```
...
# 将 GWR 的结果对象 SDF 转为 sf 格式再绘图
tm_shape(st_as_sf(atl.ss$SDF)) +
  tm_fill(
    col = c(
      'pctPOV_Median',
      'pctNOHS_Median',
    ...
    style = 'quantile',
    palette = "YlOrRd" # 建议加上色带，通常局部中位数用渐变色表示
  ) +
  tm_borders(alpha = 0.3) +
  tm_facets(ncol = 2) # 2x2 排版查看四个变量的空间异质性
```
...
tm_shape(st_as_sf(atl.ss$SDF)) +
  tm_fill(
    col = c('pctPOV_IQR', 'pctNOHS_IQR', 'pctMOVE_IQR', 'pctOWNER_OCC_IQR'),
    style = 'quantile',
    palette = "Blues" # IQR 通常使用单色系渐变，表示变异程度从低到高
  ) +
...
```{r}
# 确保显著性边界也是 sf 格式（如果尚未转换）
atl_sig90_sf <- st_as_sf(atl.sig90)
atl_sig95_sf <- st_as_sf(atl.sig95)
...
tm_shape(st_as_sf(atl.ss$SDF)) +
  ...
  tm_borders(alpha = 0.3) +
tm_shape(atl_sig90_sf) +
  tm_borders(lwd = 2, col = 'blue') +
tm_shape(atl_sig95_sf) +
  tm_borders(lwd = 2, col = 'red') +
tm_add_legend(
    type = 'lines',       # v4 建议使用 'lines' (复数)
    labels = c('p < 0.05', 'p < 0.10'),
    col = c('red', 'blue'), # v4 内部有时会自动映射 col 到线颜色
    title = "Significance"
  )
...
```{r warning=F, message=F}
# 确保使用的是 sf 对象
atl_corr_sf <- st_as_sf(atl.ss$SDF)

tm_shape(atl_corr_sf) +
  tm_fill(
    fill = c(
      'Spearman_rho_pctPOV.pctNOHS',
      'Spearman_rho_pctPOV.pctMOVE',
      'Spearman_rho_pctPOV.ICE_INCOME_all'
    ),
    # v4 语法修正：使用 values 代替 palette
    fill.scale = tm_scale_intervals(
      style = "quantile", 
      midpoint = 0,
      values = "-RdBu"  # 这里从 palette 改为 values
    ),
    fill.free = TRUE
  ) +
  tm_borders(alpha = 0.2) +
...
```{r warning = F}
sdf_sf <- st_as_sf(atl.ss$SDF)

# 3. 绘图：全部使用 sf 对象
tm_shape(sdf_sf) +
  tm_fill(
    fill = 'Spearman_rho_pctPOV.pctMOVE',
    fill.scale = tm_scale_intervals(
      style = 'quantile',
      midpoint = 0,      # 相关系数建议设 0 为中点
      values = "-RdBu"   # 红色正相关，蓝色负相关
    ),
    fill.legend = tm_legend(title = 'Local correlation Poverty\n& Residential instability')
  ) +
  tm_borders(alpha = 0.2) +
tm_shape(atl_sig90_sf) +
  tm_borders(lwd = 2, col = 'blue') +
tm_shape(atl_sig95_sf) +
  tm_borders(lwd = 2, col = 'red') +
tm_add_legend(
    type = 'lines',      # v4 必须用复数 lines
    labels = c('p < 0.05', 'p < 0.10'),
    fill = c('red', 'blue'), # v4 使用 fill 定义线条颜色
    title = "Significance"
  )
```
