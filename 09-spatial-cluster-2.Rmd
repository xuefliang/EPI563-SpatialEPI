...
sti %>%
  ...
  tm_layout(
    main.title = 'STI per capita, Georgia, 2018',
...
tibble(
  ...
  pval = format(sapply(k1$secondary.clusters, '[[', 8), digits = 3)
) %>% # this gets p.values
...
sti_scan2 <- sti_long %>%
  mutate(
    count = STD, # event variable must be labeled 'count'
  ...
  ) %>% # time-period must be labeled 'time'
...
k2 <- scan_pb_poisson(sti_scan2, zones = zones, n_mcsim = 499)
print(k2)
```
...
tm_shape(sti) +
  tm_fill('cluster', style = 'cat', palette = 'Set1') +
  tm_borders()
```
...
g +
  geom_segment(aes(yend = order), xend = 2019, size = 2) +
  geom_point(size = 3) +
  labs(x = 'Cluster timing', y = 'Cluster number', col = 'Cluster')
