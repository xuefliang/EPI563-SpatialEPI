...
m1 <- vlbw %>%
  tm_shape() +
  tm_fill('rate', style = 'quantile', palette = 'BuPu', title = '% VLBW') +
  tm_borders() +
  tm_layout(
    main.title = 'All data',
  ...
  tm_credits(
    "Very low birth weight (<1500 grams), 2018-2019, \nNo suppression",
...
m2 <- vlbw %>%
  ...
  tm_borders() +
  tm_layout(
    main.title = 'Suppress sparse counts',
    inner.margins = c(0.1, 0.02, 0.1, 0.05),
    legend.format = list(digits = 1)
  ) +
  tm_credits(
...
vlbw <- vlbw %>%
  mutate(
    expect = risk * TOT, # calculate the EXPECTED count for each county
    SMR = VLBW / expect
  ) # calculate the SMR as observed/expected
...
m1 <- vlbw %>%
  ...
  tm_borders() +
  tm_layout(
    main.title = 'Risk of VLBW in Georgia',
...
smr_map <- tm_shape(vlbw) +
  ...
  tm_borders() +
  tm_layout(
    main.title = 'SMR of VLBW in Georgia',
    inner.margins = c(0.02, 0.02, 0.1, 0.05)
  ) +
  tm_shape(ga) +
  tm_borders(lwd = 2, col = 'black')
...
df3 <- data.frame(
  x = rnbinom(10000, mu = 10, size = 2),
  Distribution = 'Neg Bin'
)
...
smr_map <- tm_shape(vlbw) +
  ...
  tm_borders() +
  tm_layout(
    main.title = 'VLBW in Georgia, 2018-2019',
...
tm_shape(vlbw) +
  ...
  tm_borders() +
  tm_layout(
    main.title = 'SMR of VLBW, GA 2018-2019',
  ...
  tm_credits(
    'Counties with higher than expected risk (p<0.05) highlighted with dark borders'
  ) +
  tm_shape(ga_counties) +
...
age %>%
  arrange(GEOID) %>%
  dplyr::select(GEOID, NAME, AGE_GROUP, VLBW, TOT) %>%
  st_drop_geometry() %>%
  head()
```
...
age <- age %>%
  arrange(GEOID, AGE_GROUP)

# Calculate the age-adjusted expected counts
vlbw$expected_indirect <- SpatialEpi::expected(
  population = age$TOT,
  cases = age$VLBW,
  n.strata = 7 #分层，age_groups 是 7 个
)
...
eb_map <- tm_shape(vlbw) +
  ...
  tm_borders() +
  tm_layout(
    main.title = 'EB smoothed SMR of VLBW',
    inner.margins = c(0.02, 0.02, 0.1, 0.05),
    legend.format = list(digits = 2)
  ) +
  tm_shape(ga_counties) +
...
ggplot(
  vlbw,
  aes(x = rate / 100, y = ebrate, color = log(shrinkage), size = VLBW)
) +
  geom_point() +
...
m4 <- tm_shape(vlbw) +
  ...
  tm_borders() +
  tm_layout(
    main.title = 'Empirical Bayes smoothed\nSMR of VLBW',
    inner.margins = c(0.1, 0.02, 0.05, 0.2),
    legend.format = list(digits = 2)
  ) +
  # Add dark borders for significant
