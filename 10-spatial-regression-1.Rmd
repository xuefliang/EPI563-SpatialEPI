...
pacman::p_load(
  ...
  tmap, # Mapping
  MASS
) # Statistical package including function for studentized residuals
```
...
vlbw <- vlbw %>%
  mutate(
    # MCD: 产科荒漠 (分类变量)
    MCD = factor(sample(
      c("Full Access", "Moderate Access", "Maternity Care Desert"),
      size = n(),
      replace = TRUE,
      prob = c(0.5, 0.3, 0.2)
    )),
    ...
    # PctPov: 贫困率 (0-1)
    PctPov = runif(n(), 0.05, 0.45),

    # isolation: 黑白居住隔离指数 (0-1)
    isolation = runif(n(), 0.1, 0.9),

    # FI: 粮食不安全指数
    FI = runif(n(), 0.08, 0.25),

    # SocCap: 社会资本指数 (标准正态分布)
    SocCap = rnorm(n(), mean = 0, sd = 1),

    # pctNOIns_Fem: 女性无保险比例 (0-1)
    pctNOIns_Fem = runif(n(), 0.02, 0.15)
  )
```
...
m0 <- lm(rate ~ 1, data = vlbw, weights = wts)
...
```{r}
m1 <- lm(
  rate ~ PctPov + isolation + pctNOIns_Fem + MCD,
  data = vlbw,
  weights = wts
)
summary(m1)
...
moran.test() 默认输入的序列是相互独立的观测值。然而，回归残差 $\epsilon = Y - X\beta$ 并不是完全独立的。
...
```{r}
g0 <- glm(VLBW ~ 1 + offset(log(TOT)), data = vlbw, family = poisson())
g1 <- glm(
  VLBW ~ PctPov + isolation + pctNOIns_Fem + MCD + offset(log(TOT)),
  data = vlbw,
  family = poisson()
)
```
