...
```{r include = F}
atl <- readRDS('data/atl.rds') |>
  mutate(
    # 模拟 SMOKING 变量：
    # 基础吸烟率设定在 15% 左右
    # 加上 3 * Poverty_std 模拟社会经济因素的影响
    # 加上 rnorm 模拟其他未观测到的随机波动
    SMOKING = 15 + (3 * Poverty_std) + rnorm(n(), 0, 1.5),

    # 确保吸烟率在合理范围内（例如不低于 5%，不高于 40%）
    SMOKING = pmax(pmin(SMOKING, 40), 5)
  )
...
h.fixed <- bw.gwr(MENTALHLTH ~ SMOKING + PHYSHLTH, data = atl.sp, dMat = atl.DM)

h.adapt <- bw.gwr(
  MENTALHLTH ~ SMOKING + PHYSHLTH,
  data = atl.sp,
  adaptive = T,
  dMat = atl.DM
)
...
```{r message=F}
collin_sf <- st_as_sf(collin$SDF)

tm_shape(collin_sf) +
  tm_polygons(
    fill = "local_CN",
    fill.scale = tm_scale_intervals(
      values = "prgn", # v4 建议直接使用名称，反转颜色可用 midpoint 或手动传参
      breaks = c(0, 15, 25, 30, 35, Inf) # 加上 Inf 解决“值高于最高间断点”的警告
    ),
    fill_alpha = 0.2, # 对应 v4 的新参数名
    col = "black", # 边界颜色
    col_alpha = 0.1 # 边界透明度
  )
```
...
tm_shape(collin_sf) +
  tm_polygons(
    fill = c("SMOKING_VIF", "PHYSHLTH_VIF"), # 依然可以传向量，但控制逻辑变了
    fill.scale = tm_scale_intervals(
      values = "brewer.prgn", # 使用完整调色板名称
      breaks = c(0, 1, 2, 3, 4, Inf) # 加上 Inf 防止超出范围报错
    ),
    fill_alpha = 0.2
  ) +
  tm_facets(nrow = 1)
...
smoke.prev <- tm_shape(atl) +
  tm_polygons(
    ...
    fill.scale = tm_scale_intervals(
      style = "quantile",
      values = "brewer.bu_pu" # v4 推荐使用完整前缀避免歧义
    ),
    fill.legend = tm_legend(
      title = "%"
    )
  ) +
  tm_title("Prevalence of smoking") + # 标题从 tm_layout 移出
  tm_layout(
    legend.format = list(digits = 1)
  )

# Map for spatially varying SMOKING coefficients
smoke.coefficients <- tm_shape(st_as_sf(m$SDF)) +
  tm_polygons(
    fill = "SMOKING",
    fill.scale = tm_scale_intervals(
      style = "quantile",
      midpoint = NA # v4 默认会对正负值设置中心点，设为 NA 可使用连续分段
    ),
    fill.legend = tm_legend(title = "\u03B2"), # 标题移入 fill.legend
    fill_alpha = 0.2
  ) +
  tm_shape(st_as_sf(smoke.p)) +
  tm_borders(col = "red", lwd = 1) +
  tm_title("Coefficients for SMOKING") + # 使用 tm_title 代替 tm_layout
  tm_layout(
    legend.format = list(digits = 2)
  ) +
  tm_add_legend(
    type = "lines", # 从 "line" 变为 "lines"
    labels = "p-value < 0.05",
    fill = "red" # 从 col 变为 fill
  )
...
```{r message=F, warning=F}
m_sdf <- st_as_sf(m$SDF)
# Map for spatially varying intercepts
int.coefficients <- tm_shape(m_sdf) +
  tm_fill('Intercept', style = 'quantile', title = '\u03B2', palette = 'OrRd') +
  tm_borders(alpha = 0.2) +
  tm_shape(intercept.p) +
  tm_borders(col = 'red', lwd = 1) +
  tm_layout(title = 'Intercepts', legend.format = list(digits = 2)) +
  tm_add_legend(type = 'line', labels = 'p-value < 0.05', col = 'red')

# Map for spatially varying PHYSHLTH coefficients
physhlth.coefficients <- tm_shape(m_sdf) +
  tm_fill(
    'PHYSHLTH',
    style = 'quantile',
    title = '\u03B2',
  ...
  tm_borders(col = 'red', lwd = 1) +
  tm_layout(
    title = 'Coefficients for PHYSHLTH',
    legend.format = list(digits = 2)
  ) +
  tm_add_legend(type = 'line', labels = 'p-value < 0.05', col = 'red')
...
res <- 1000
c.dim <- c(
  ceiling((bb[1, 2] - bb[1, 1]) / res),
  ceiling((bb[2, 2] - bb[2, 1]) / res)
)

grd <- SpatialGrid(GridTopology(
  cellcentre.offset = bb[, 1],
  cellsize = c(res, res),
  cells.dim = c.dim
))
```
...
```{r}
atl.DM2 <- gw.dist(dp.locat = coordinates(atl.sp), rp.locat = coordinates(grd))
```
...
atl_proj <- st_transform(atl, 26916) # 转换为米制单位

# 2. 重新获取投影后的 Bbox 和网格
bb <- st_bbox(atl_proj)
res <- 1000 # 此时 1000 代表 1000米

# 3. 创建网格（使用投影后的坐标）
grid_sf <- st_make_grid(atl_proj, cellsize = c(res, res), what = "centers")
grd_pixels <- as(as_Spatial(grid_sf), "SpatialPixels")

# 4. 转换回 Spatial 用于 GWmodel
atl_sp_proj <- as_Spatial(atl_proj)

# 5. 重新计算距离和模型
atl.DM2 <- gw.dist(
  dp.locat = coordinates(atl_sp_proj),
  rp.locat = coordinates(grd_pixels)
)
...
raster.int <- tm_shape(m2.raster$Intercept) +
  tm_raster('Intercept', style = 'cont', palette = 'OrRd')

raster.smoke <- tm_shape(m2.raster$SMOKING) +
  tm_raster('SMOKING', style = 'cont', palette = 'BuPu')

raster.phyhlth <- tm_shape(m2.raster$PHYSHLTH) +
  tm_raster('PHYSHLTH', style = 'cont', palette = 'YlGnBu')
