...
pacman::p_load(
  tidyverse, # Provides data manipulation and piping functionality
...
```{r}
moran.test(vlbw$rate, listw = q_listw, randomisation = T)
```
...
当样本量小的时候，moran.test 不准，moran.mc 比较准.moran.test(randomisation=T) → 近似

```{r}
moran.mc(vlbw$rate, listw = q_listw, nsim = 999)
```
...
```{r message = F, warning=F}
ebm <- EBImoran.mc(n = vlbw$VLBW, x = vlbw$TOT, listw = q_listw, nsim = 499)
print(ebm)
```
...
```{r}
lm1 <- localmoran(x = vlbw$rate, listw = q_listw)
...
vlbw_lm <- vlbw %>%
  mutate(
    raw_std = as.numeric(scale(rate)), # scale means standardize to mean 0, 1 SD
    lag_std = as.numeric(scale(lag)),
    lm_quad = factor(
      case_when(
        # All of this is assigning labels based on values
...
localmoran.exact()
`
每个县的样本量都很小，普通检验不准，所以用 “精确检验”
必须输入一个 线性回归模型（lm），可以用人口加权，让大县更稳，用邻接关系（nb） 而不是权重矩阵
...
For this approach, you must supply a *regression model object* because the test is done on the model-fitted values. The help documentation specifies this must be an `
lm()
` object (e.g. linear model), and that no offsets are allowed. 

One way to get the linear regression to account for the difference in population-at-risk is to weight each observation (e.g. each region) such that their weight is the proportion of the total state population of births at risk. Also note, that `
localmoran.exact()
` uses the *spatial neighbors* objects, `
qnb
`, rather than the previously used *spatial neighbors weights list*, `
q_listw
`.
...
reg1 <- lm(rate ~ 1, data = vlbw, weights = wts)
...
```{r}
#' Local Moran's I Exact Test
#'
#' Perform exact local Moran's I test using weighted linear regression approach.
#' This function uses spatial neighbors objects and population-weighted regression
#' to conduct exact local spatial autocorrelation tests.
#'
#' @param observed Vector of observed event counts or rates
#' @param population Vector of population at risk (e.g., total births)
#' @param nb Spatial neighbors object (from spdep)
#' @param formula Formula for regression model (default: "observed ~ 1" for intercept-only)
#' @param weight_type Type of weighting scheme: "population" (default) or "equal"
#' @param return_model Logical, whether to return the fitted model object (default: FALSE)
#' @param alpha Significance level for determining significance (default: 0.05)
#'
#' @return A list containing:
#'   \item{I_exact}{Local Moran's I statistics}
#'   \item{E_I}{Expected values of local Moran's I}
#'   \item{pval_exact}{Exact p-values}
#'   \item{significant}{Logical vector indicating significance}
#'   \item{weights}{Weights used in regression}
#'   \item{fitted_model}{Fitted regression model (if requested)}
#'   \item{summary}{Summary statistics}
#'
#' @examples
#' # Basic usage with rate data
#' result <- local_moran_exact(
#'   observed = vlbw$rate,
#'   population = vlbw$TOT,
#'   nb = qnb
#' )
#'
#' # Usage with count data
#' result <- local_moran_exact(
#'   observed = vlbw$VLBW,
#'   population = vlbw$TOT,
#'   nb = qnb
...
#'
#' # Access results
#' print(result$I_exact)
#' print(result$pval_exact)
#' print(sum(result$significant))

local_moran_exact <- function(
  observed,
  population,
  nb,
  formula = NULL,
  ...
  # Input validation
  if (length(observed) != length(population)) {
    stop("observed and population vectors must have the same length")
  }
  ...
  if (any(is.na(observed)) || any(is.na(population))) {
    stop("observed and population vectors cannot contain NA values")
  }

  if (!inherits(nb, "nb")) {
    stop("nb must be a spatial neighbors object from spdep")
  }

  # Check if spdep package is available
  if (!requireNamespace("spdep", quietly = TRUE)) {
    stop("spdep package is required for this function")
  }

  N <- length(observed)

  # Create weights based on relative population size
  if (weight_type == "population") {
    weights <- population / sum(population) * N
  } else if (weight_type == "equal") {
    weights <- rep(1, N)
  } else {
    stop("weight_type must be either 'population' or 'equal'")
  }
  ...
  # Set up formula
  if (is.null(formula)) {
    formula <- observed ~ 1 # Intercept-only model
  } else {
    formula <- as.formula(formula)
  }

  # Fit weighted linear regression model
  cat("Fitting weighted linear regression model...\n")
  fitted_model <- lm(formula, data = reg_data, weights = weights)

  # Perform exact local Moran's I test
  cat("Performing exact local Moran's I test...\n")

  tryCatch(
    {
      exact_result <- spdep::localmoran.exact(fitted_model, nb = nb)
    },
    error = function(e) {
      stop("Error in localmoran.exact(): ", e$message)
    }
  )

  # Convert to matrix format for easier extraction
  exact_matrix <- print(exact_result)

  # Extract results
  I_exact <- exact_matrix[, 1] # Local Moran's I statistics
  E_I <- exact_matrix[, 2] # Expected values
  pval_exact <- exact_matrix[, 3] # Exact p-values

  # Determine significance
  significant <- pval_exact < alpha

  # Create summary statistics
  summary_stats <- data.frame(
    min_I = min(I_exact, na.rm = TRUE),
    max_I = max(I_exact, na.rm = TRUE),
    mean_I = mean(I_exact, na.rm = TRUE),
    sd_I = sd(I_exact, na.rm = TRUE),
    min_pval = min(pval_exact, na.rm = TRUE),
    max_pval = max(pval_exact, na.rm = TRUE),
  ...
  results <- list(
    ...
    exact_result_matrix = exact_matrix
  )

  # Optionally include fitted model
  if (return_model) {
    results$fitted_model <- fitted_model
  }

  cat("Exact local Moran's I test complete.\n")
  cat(
  ...
  cat(
    ...
    round(mean(significant, na.rm = TRUE), 4),
    "\n"
  )

  class(results) <- "local_moran_exact"
  return(results)
}

# 添加打印方法
print.local_moran_exact <- function(x, ...) {
  cat("Local Moran's I Exact Test\n")
  cat("==========================\n")
  cat("Number of regions:", x$n_regions, "\n")
  cat("Weight type:", x$weight_type, "\n")
  cat("Formula:", deparse(x$formula), "\n")
  cat("Significance level:", x$alpha, "\n")
  cat("Number of significant regions:", x$n_significant, "\n")
  cat(
    "Proportion significant:",
    round(x$n_significant / x$n_regions, 4),
    "\n\n"
  )
...
# 添加汇总方法
summary.local_moran_exact <- function(object, ...) {
  cat("Local Moran's I Exact Test - Summary\n")
  cat("====================================\n")
  cat("Regions analyzed:", object$n_regions, "\n")
  cat("Weight type:", object$weight_type, "\n")
  cat("Formula:", deparse(object$formula), "\n")
  cat("Alpha level:", object$alpha, "\n\n")

  cat("Local Moran's I Statistics:\n")
  cat("Min:", round(min(object$I_exact, na.rm = TRUE), 4), "\n")
  cat("1st Qu.:", round(quantile(object$I_exact, 0.25, na.rm = TRUE), 4), "\n")
  cat("Median:", round(median(object$I_exact, na.rm = TRUE), 4), "\n")
  cat("Mean:", round(mean(object$I_exact, na.rm = TRUE), 4), "\n")
  cat("3rd Qu.:", round(quantile(object$I_exact, 0.75, na.rm = TRUE), 4), "\n")
  cat("Max:", round(max(object$I_exact, na.rm = TRUE), 4), "\n\n")

  cat("Expected Values:\n")
  cat("Min:", round(min(object$E_I, na.rm = TRUE), 4), "\n")
  cat("Median:", round(median(object$E_I, na.rm = TRUE), 4), "\n")
  cat("Mean:", round(mean(object$E_I, na.rm = TRUE), 4), "\n")
  cat("Max:", round(max(object$E_I, na.rm = TRUE), 4), "\n\n")

  cat("P-values:\n")
  cat("Min:", round(min(object$pval_exact, na.rm = TRUE), 4), "\n")
  cat(
    "1st Qu.:",
    round(quantile(object$pval_exact, 0.25, na.rm = TRUE), 4),
    "\n"
  )
  cat("Median:", round(median(object$pval_exact, na.rm = TRUE), 4), "\n")
  cat("Mean:", round(mean(object$pval_exact, na.rm = TRUE), 4), "\n")
  cat(
    "3rd Qu.:",
    round(quantile(object$pval_exact, 0.75, na.rm = TRUE), 4),
    "\n"
  )
  cat("Max:", round(max(object$pval_exact, na.rm = TRUE), 4), "\n\n")
  ...
  cat(
    "Proportion significant:",
    round(object$n_significant / object$n_regions, 4),
    "\n"
  )

  if (object$n_significant > 0) {
    sig_indices <- which(object$significant)
    cat(
      "Significant region indices:",
      paste(sig_indices, collapse = ", "),
      "\n"
    )
  }
}
...
# 添加提取方法，用于获取特定组件
extract_results <- function(x, component = "all") {
  if (!inherits(x, "local_moran_exact")) {
    stop("x must be a local_moran_exact object")
  }
  ...
  switch(
    ...
    "weights" = x$weights,
    "matrix" = x$exact_result_matrix,
    "all" = x,
    stop(
      "component must be one of: 'I', 'pval', 'significant', 'weights', 'matrix', 'all'"
    )
  )
}
```
...
```{r}
result_exact <- local_moran_exact(
  observed = vlbw$rate,
  population = vlbw$TOT,
  nb = qnb
)
# 查看结果
print(result_exact)
summary(result_exact)

# 将结果添加到数据框
vlbw_lm$I_exact <- result_exact$I_exact
vlbw_lm$pval_exact <- result_exact$pval_exact
vlbw_lm$significant_exact <- result_exact$significant
```
...
tm_shape(vlbw_lm2) +
  tm_fill(
    c('lm_quad', 'lm_quad_exact', 'lm_quad_CR'),
    style = 'cat',
    palette = c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#ffffb3"),
...
标准 LISA（localmoran）：普通，直接用原始率，小样本不准
精确局部莫兰（localmoran.exact）：，解决每个县样本量小的问题，更稳
固定风险 Poisson 局部莫兰（Constant Risk）： 最适合罕见病 + 小人口县，用期望数模拟，排除随机波动
...
#' Local Moran's I with Constant Risk Assumption
#'
#' Calculate local Moran's I statistics under the constant risk hypothesis
#' for Poisson-distributed case event counts over population at risk.
#'
#' @param observed Vector of observed event counts
#' @param expected Vector of expected event counts under constant risk
#' @param listw Spatial weights list object (from spdep)
#' @param nsim Number of simulations for null hypothesis testing (default: 499)
#' @param seed Random seed for reproducibility (default: 123)
#' @param alpha Significance level for p-value calculation (default: 0.05)
#' @param return_simulations Logical, whether to return simulation matrix (default: FALSE)
#'
#' @return A list containing:
#'   \item{I_CR}{Local Moran's I statistics under constant risk}
#'   \item{pval_CR}{P-values from simulation}
#'   \item{significant}{Logical vector indicating significance at alpha level}
#'   \item{standardized_deviation}{Standardized deviation of observed from expected}
#'   \item{spatial_lag}{Spatially lagged standardized deviation}
#'   \item{summary}{Summary statistics}
#'   \item{simulation_results}{Matrix of simulation results (if requested)}
#'
#' @examples
#' # Assuming you have observed counts, expected counts, and spatial weights
#' result <- local_moran_constant_risk(
#'   observed = vlbw$VLBW,
#'   expected = vlbw$expected,
#'   listw = q_listw,
#'   nsim = 999
#' )
#'
#' # Access results
#' print(result$I_CR)
#' print(result$pval_CR)
#' print(sum(result$significant))  # Count of significant regions

local_moran_constant_risk <- function(
  observed,
  expected,
  listw,
  nsim = 499,
  ...
  if (any(expected <= 0)) {
    stop("expected values must be positive")
  }

  if (any(observed < 0)) {
    stop("observed values must be non-negative")
  }
  ...
  # Step 1: Create standardized deviation of observed from expected
  sd_CR <- (observed - expected) / sqrt(expected)

  # Step 2: Create spatially lagged version of standardized deviation
  wsd_CR <- spdep::lag.listw(listw, sd_CR)

  # Step 3: Calculate local Moran's I under constant risk
  I_CR <- sd_CR * wsd_CR

  # Step 4: Setup simulation parameters
  sims <- matrix(0, ncol = nsim, nrow = N)

  # Step 5: Simulate null distribution
  cat("Running", nsim, "simulations...\n")

  for (i in 1:nsim) {
    # Generate random Poisson counts given expected values
    y_sim <- rpois(N, lambda = expected)

    # Calculate standardized deviation for simulation
    sd_CR_sim <- (y_sim - expected) / sqrt(expected)

    # Calculate spatially lagged standardized deviation
    wsd_CR_sim <- spdep::lag.listw(listw, sd_CR_sim)

    # Calculate local Moran's I for this simulation
    sims[, i] <- sd_CR_sim * wsd_CR_sim
  }

  # Step 6-8: Calculate p-values
  # For each region, rank the observed value against simulated values
  rank_results <- apply(cbind(I_CR, sims), 1, function(x) rank(x)[1])

  # Calculate differences and p-values
  diff <- pmax(0, nsim - rank_results)
  pval_CR <- punif((diff + 1) / (nsim + 1))

  # Determine significance
  significant <- pval_CR < alpha
  ...
  # Optionally include simulation results
  if (return_simulations) {
    results$simulation_results <- sims
  }
  ...
  results$summary <- data.frame(
    ...
    mean_I = mean(I_CR),
    sd_I = sd(I_CR),
    min_pval = min(pval_CR),
    max_pval = max(pval_CR),
    n_significant = sum(significant),
    prop_significant = mean(significant)
  )

  cat("Simulation complete.\n")
  cat("Number of significant regions:", sum(significant), "out of", N, "\n")
  cat("Proportion significant:", round(mean(significant), 4), "\n")

  class(results) <- "local_moran_cr"
  return(results)
}

# 添加一个打印方法
print.local_moran_cr <- function(x, ...) {
  cat("Local Moran's I with Constant Risk Assumption\n")
  cat("=============================================\n")
  cat("Number of regions:", length(x$I_CR), "\n")
  cat("Number of simulations:", x$nsim, "\n")
  cat("Significance level:", x$alpha, "\n")
  cat("Number of significant regions:", x$n_significant, "\n")
  cat(
    "Proportion significant:",
    round(x$n_significant / length(x$I_CR), 4),
    "\n\n"
  )
...
# 添加一个简单的汇总方法
summary.local_moran_cr <- function(object, ...) {
  cat("Local Moran's I with Constant Risk - Summary\n")
  cat("===========================================\n")
  cat("Regions analyzed:", length(object$I_CR), "\n")
  cat("Simulations run:", object$nsim, "\n")
  cat("Alpha level:", object$alpha, "\n\n")

  cat("Local Moran's I Statistics:\n")
  cat("Min:", round(min(object$I_CR), 4), "\n")
  cat("1st Qu.:", round(quantile(object$I_CR, 0.25), 4), "\n")
  cat("Median:", round(median(object$I_CR), 4), "\n")
  cat("Mean:", round(mean(object$I_CR), 4), "\n")
  cat("3rd Qu.:", round(quantile(object$I_CR, 0.75), 4), "\n")
  cat("Max:", round(max(object$I_CR), 4), "\n\n")

  cat("P-values:\n")
  cat("Min:", round(min(object$pval_CR), 4), "\n")
  cat("1st Qu.:", round(quantile(object$pval_CR, 0.25), 4), "\n")
  cat("Median:", round(median(object$pval_CR), 4), "\n")
  cat("Mean:", round(mean(object$pval_CR), 4), "\n")
  cat("3rd Qu.:", round(quantile(object$pval_CR, 0.75), 4), "\n")
  cat("Max:", round(max(object$pval_CR), 4), "\n\n")
  ...
  cat(
    "Proportion significant:",
    round(object$n_significant / length(object$I_CR), 4),
    "\n"
  )
...
# 将结果添加到数据框
vlbw_lm$I_CR <- result$I_CR
vlbw_lm$pval_CR <- result$pval_CR
vlbw_lm$significant_CR <- result$significant

# 获取显著区域的信息
significant_regions <- which(result$significant)
cat("Significant regions:", significant_regions, "\n")
```